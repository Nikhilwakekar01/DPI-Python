...
The Python implementation reproduces the active C++ `src/dpi_mt.cpp` pipeline:
...
```bash
python -m python_dpi INPUT_PCAP OUTPUT_PCAP
```

For example:

```bash
python -m python_dpi Packet_analyzer/test_dpi.pcap output_python.pcap
```

The Python engine is intended to reproduce the behavior of the active C++
`dpi_mt.cpp` implementation, including packet parsing, flow affinity,
classification, blocking, and PCAP output.

### Tests

```bash
py -3 -m unittest discover -s python_dpi/tests -p "test_*.py" -v
```
